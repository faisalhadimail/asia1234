...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: allPromos } = await supabase.from('Promo').select('*')
    const promoMap = new Map<string, string>()
    ;(allPromos || []).forEach((p) => promoMap.set(p.badge.toUpperCase(), p.id))
    ...
    for (let r = headerRowIndex + 1; r <= ws.rowCount && processed < MAX_ROWS; r++) {
      ...
      try {
        const { data: newProperty, error } = await supabase.from('Property').insert({
          title,
          price,
          dp,
          allInCost,
        ...
        if (promoIds.length > 0) {
          await supabase.from('PropertyPromo').insert(
            promoIds.map((promoId) => ({ propertyId: newProperty.id, promoId }))
          )
        }
        ...
        if (message.includes('duplicate') || message.includes('unique')) {
          result.details.push({
            ...
            title,
            status: 'gagal',
            error: `Permalink "${finalPermalink}" sudah digunakan. Gunakan permalink lain.`,
          })
        } else {
          result.details.push({
            row: r,
