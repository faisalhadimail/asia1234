...
interface AppState {
  ...
  editingPropertyTypeIndex: number | null
  ...
  savePropertyType: (data: any, index?: number | null) => Promise<void>
  deletePropertyType: (index: number) => Promise<void>
...
export const useStore = create<AppState>((set, get) => ({
  ...
  // Property Types
  savePropertyType: async (data, index) => {
    const { showLoading, hideLoading, fetchAllData } = get()
  ...
  deletePropertyType: async (index) => {
    const { showLoading, hideLoading, fetchAllData } = get()
    showLoading('Menghapus jenis properti...')
    try {
      await fetch(`/api/property-types?index=${index}`, { method: 'DELETE' })
      await fetchAllData()
    } catch (e) {
      console.error(e)
    } finally {
